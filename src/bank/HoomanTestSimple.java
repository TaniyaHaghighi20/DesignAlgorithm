...
public class HoomanTestSimple {
    public static void main(String[] args) throws IOException {
        ...
        coordinator.experiment(10000, 50);
...
//        bank2.reportRanges(rangesList);
        ...
        bank4.reportRanges(rangesList);



        //-------------------------------------------------------------------------------------------------------------
        Bank5 bank5 = new Bank5("Hooman Bank 4");

        for (Account ac : accounts) {
            if (!bank5.addAccount(ac)) {
                System.out.println("error in adding accounts");
                System.exit(0);
            }
        }
        ...
        System.out.println("\n\nTotal balance = " + bank5.calcTotalBalance());

        //test search (valid accounts)
        for (int id = 1; id <= 4; id++) {
            Account ac = bank5.findAccount(id);
            if (ac == null || !(ac.getName().equals(accounts.get(id - 1).getName()))
                    || ac.getID() != accounts.get(id - 1).getID()
                    || !(ac.getCity().equals(accounts.get(id - 1).getCity()))) {
                System.out.println("error in searching valid accounts");
        ...
        System.out.println("Total balance after withdraw = " + bank5.calcTotalBalance());

        //test search (invalid accounts)
        for (int id = 5; id <= 1000; id++) {
            if (bank5.findAccount(id) != null) {
                System.out.println("error in searching invalid accounts");
                System.exit(0);
            }
        }
        ...
        //test city aggregation-Balance
        BinarySearchTreeForBank5<String, Double> balances4 = bank5.getTotalBalancePerCity();

        //test city aggregation-Count
        BinarySearchTreeForBank5<String, Integer> counts4 = bank5.getTotalCountPerCity();

        //test range aggregation-Count
        ArrayList<Integer> countsPerRange4 = bank5.getTotalCountPerRange(rangesList);

        // test count aggregation
        for (int k = 0; k < counts.size(); k++) {
            if (Integer.compare(counts4.search(cities.get(k)), counts.get(k)) != 0) {
                System.out.println("error in city-count aggregation");
                System.exit(0);
            }
        }
        ...
        // test balance aggregation
        for (int k = 0; k < balances.size(); k++) {
            if (Double.compare(balances4.search((cities.get(k))), balances.get(k)) != 0) {
                System.out.println("error in city-balance aggregation");
                System.exit(0);
            }
        }
        ...
        // test range aggregation
        for (int k = 0; k < countsPerRange.size(); k++) {
            if (Integer.compare(countsPerRange4.get(k), countsPerRange.get(k)) != 0) {
                System.out.println("error in range-count aggregation");
                System.exit(0);
            }
        }

        bank5.reportCity(cities, balances4, counts4);
        bank5.reportRanges(rangesList);*/
    }
}